...
const toggleSelectAll = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    citizenship: 'all',
    supervisor: 'all', // Changed from requestedBy to supervisor
    searchTerm: '',
...
const fetchRecords = async (searchTerm = filters.searchTerm, currentFilters = filters) => {
  ...
  try {
    ...
    // Apply date range filtering - add this after your other filters
if (currentFilters.startDate || currentFilters.endDate) {
  ...
  clientFilteredData = clientFilteredData.filter(item => {
    ...
    // Apply filter logic - checking for overlapping date ranges
  ...
  const exportToExcel = async () => {
    try {
      ...
      const exportData = exportRecords.map((record, index) => ({
        ...
        'Additional Info': record.additional_info || 'N/A',
        'Created By': record.created_by || 'N/A',
        'Created At': record.created_at ? format(new Date(record.created_at), 'yyyy-MM-dd HH:mm') : 'N/A'
      }));
      ...
      // Create filename based on the current filters
      ...
      if (filters.department_name !== 'all') {
        const deptName = departments.find(d => d.id === parseInt(filters.department_name))?.name || filters.department_name;
        fileName += `_${deptName}`;
      }
      ...
      if (apiService.activityLog && typeof apiService.activityLog.logActivity === 'function') {
        await apiService.activityLog.logActivity({
          ...
          description: `Exported internship records with filters: ${JSON.stringify(filters)}`,
          type:'export'
        });
      }
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {loading ? (
                      ...
                      displayedRecords.map((record, index) => {
                        ...
                        // Calculate time remaining for Active internships
                        let timeRemaining = null;
                        if (record.status === 'Active' && record.date_end) {
                        ...
                        return (
                          <tr 
                            ...
                            onClick={() => navigate(`/update-internship-request/${record.id}`)}
                          >
...
export default AllInternshipChecks;