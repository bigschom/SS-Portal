...
const AllInternshipBackgroundChecks = () => {
  ...
  // Fetch requesters for internship
  const fetchRequesters = async () => {
    ...
    try {
      ...
      const uniqueRequesters = [...new Set(
        response
          .filter(item => item.role_type === 'Internship')
  ...
  // Fetch status options for internship
  ...
  const fetchRecords = async (searchTerm = filters.searchTerm, currentFilters = filters) => {
    ...
    setError(null);
    try {
      // Specifically fetch only Internship records
      const allInternshipRequests = (await apiService.backgroundChecks.getAllBackgroundChecks())
        .filter(item => item.role_type === 'Internship');
      
      if (allInternshipRequests.error) {
        throw new Error(allInternshipRequests.error);
      }
      
      // Apply filters
      let filteredData = allInternshipRequests;
      ...
      filteredData.sort((a, b) => {
        ...
        // Handle date fields
        if (['submitted_date', 'date_start', 'date_end'].includes(sortConfig.key)) {
      ...
      console.log('Filtered internship data:', filteredData.length, 'records');
      setRecords(allInternshipRequests);
      setFilteredRecords(filteredData);
      setCurrentPage(1);
    } catch (error) {
      console.error('Error fetching internship records:', error);
      setError('Failed to load internship background checks');
      showToast('Failed to load internship background checks', 'error');
    } finally {
      setLoading(false);
    }
  };

  // Export Internship Records
  const exportToExcel = async () => {
    try {
      ...
      const exportData = exportRecords.map((record, index) => ({
        ...
        'Operating Country': record.operating_country || 'N/A',
        ...
        'Submitted Date': record.submitted_date ? format(new Date(record.submitted_date), 'yyyy-MM-dd') : 'N/A'
      }));

      const ws = XLSX.utils.json_to_sheet(exportData);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Internship Background Checks');
      
      const fileName = `internship_background_checks_${format(new Date(filters.startDate), 'yyyy-MM-dd')}_to_${format(new Date(filters.endDate), 'yyyy-MM-dd')}.xlsx`;
      XLSX.writeFile(wb, fileName);

      showToast(`Exported ${exportData.length} internship records successfully`, 'success');

      // Log export activity
      if (apiService.activityLog && typeof apiService.activityLog.logActivity === 'function') {
        await apiService.activityLog.logActivity({
          userId: user.id,
          description: `Exported internship background checks from ${filters.startDate} to ${filters.endDate}`,
          type:'export'
        });
      }

    } catch (error) {
      console.error('Error exporting internship data:', error);
  ...
  // Pagination handler for items per page
                  ...
                  <thead className="bg-gray-50 dark:bg-gray-700">
                    <tr>
                      ...
                      {[
                        ...
                        { key: 'from_company', label: 'Company', width: 'w-[12%]' },
                        { key: 'operating_country', label: 'Country', width: 'w-[10%]' },
                      ...
                      displayedRecords.map((record, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-gray-800 dark:text-gray-200">
                              {record.from_company || 'N/A'}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-gray-800 dark:text-gray-200">
                              {record.operating_country || 'N/A'}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-gray-800 dark:text-gray-200">
...
export default AllInternshipBackgroundChecks;