  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (const recordId of selectedRecords) {
        await apiService.backgroundChecks.deleteBackgroundCheck(recordId);
      }
      ...
      // Show success toast
      showToast(`${selectedRecords.length} record(s) deleted successfully`, 'success');
...
const toggleSelectAll = () => {
  setSelectedRecords(
    selectedRecords.length === displayedRecords.map(r => r.id).length 
      ? [] 
        ...
        <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently delete {selectedRecords.length} background {selectedRecords.length !== 1 ? 'checks' : 'check'}. 
...
export default DeleteBackgroundChecks;