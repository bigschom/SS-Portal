...
const MomoTransactionRequest = ({ onBack }) => {
  const { user } = useAuth();
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (formData.momoTransactions?.length) {
        const transactionRequests = formData.momoTransactions.map(transaction => ({
      ...
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Request MoMo transaction details
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            {/* Right Panel - MOMO Transaction Requests */}
            <div className="lg:col-span-8 space-y-6">
              ...
              >
                <h2 className="text-xl font-semibold mb-4 dark:text-white">Transaction Requests</h2>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          <input
                            ...
                            className={`w-full px-4 py-2 rounded-lg border 
                                      ${errors.momoTransactions?.[index]?.phone_number ? 'border-red-500' : 'border-gray-200 dark:border-gray-600'}
                                      bg-white dark:bg-gray-800 dark:text-white
                                      focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent`}
                          />
                          {errors.momoTransactions?.[index]?.phone_number && (
                            <p className="mt-1 text-sm text-red-500 dark:text-red-400">
                              {errors.momoTransactions[index].phone_number.message}
                            </p>
                          )}
                        </div>
                        ...
                        <div>
                          <input
                            ...
                            className={`w-full px-4 py-2 rounded-lg border 
                                      ${errors.momoTransactions?.[index]?.start_date ? 'border-red-500' : 'border-gray-200 dark:border-gray-600'}
                                      bg-white dark:bg-gray-800 dark:text-white
                                      focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent`}
                          />
                          {errors.momoTransactions?.[index]?.start_date && (
                            <p className="mt-1 text-sm text-red-500 dark:text-red-400">
                              {errors.momoTransactions[index].start_date.message}
                            </p>
                          )}
                        </div>
                        ...
                        <div>
                          <input
                            ...
                            className={`w-full px-4 py-2 rounded-lg border 
                                      ${errors.momoTransactions?.[index]?.end_date ? 'border-red-500' : 'border-gray-200 dark:border-gray-600'}
                                      bg-white dark:bg-gray-800 dark:text-white
                                      focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent`}
                          />
                          {errors.momoTransactions?.[index]?.end_date && (
                            <p className="mt-1 text-sm text-red-500 dark:text-red-400">
                              {errors.momoTransactions[index].end_date.message}
                            </p>
                          )}
                        </div>
                      </div>
              ...
              {/* Additional Details - Now Required */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6"
              >
                ...
                <textarea
                  {...register('details', { required: 'Please explain why you are requesting this transaction history' })}
                  rows={4}
                  placeholder="Why are you requesting that MoMo Transaction history?"
                  className={`w-full px-4 py-2 rounded-lg border 
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-600 dark:text-blue-400 mt-1">
                      Please ensure you are requesting transaction history for numbers registered under your name. 
                      Transaction history can only be provided for numbers that belong to the requestor.
                    </p>
                  </div>
                </div>
              </motion.div>
