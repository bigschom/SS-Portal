...
const AgentCommissionRequest = ({ onBack }) => {
  const { user } = useAuth();
    ...
    try {
      ...
      if (formData.agentRequests?.length) {
        const agentRequests = formData.agentRequests.map(request => ({
          ...
          expected_amount: parseFloat(request.expected_amount) || 0,
          current_paid: parseFloat(request.current_paid) || 0
        }));
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white relative">
      ...
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-semibold">Agent Commission Request</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Request agent commission details
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            {/* Right Panel - Agent Commission Requests */}
            <div className="lg:col-span-8 space-y-6">
              ...
              >
                <h2 className="text-xl font-semibold mb-4 dark:text-white">Agent Commission Details</h2>
                ...
                <div className="space-y-4">
                  {fields.map((field, index) => (
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                          <input
                            ...
                            className={`w-full px-4 py-2 rounded-lg border 
                                      ${errors.agentRequests?.[index]?.number ? 'border-red-500' : 'border-gray-200 dark:border-gray-600'}
                                      bg-white dark:bg-gray-800 dark:text-white
                                      focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent`}
                          />
                          {errors.agentRequests?.[index]?.number && (
                            <p className="mt-1 text-sm text-red-500 dark:text-red-400">
                              {errors.agentRequests[index].number.message}
                            </p>
                          )}
                        </div>
                        ...
                        <div>
                          <input
                            ...
                            className={`w-full px-4 py-2 rounded-lg border 
                                      ${errors.agentRequests?.[index]?.franchisee ? 'border-red-500' : 'border-gray-200 dark:border-gray-600'}
                                      bg-white dark:bg-gray-800 dark:text-white
                                      focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent`}
                          />
                          {errors.agentRequests?.[index]?.franchisee && (
                            <p className="mt-1 text-sm text-red-500 dark:text-red-400">
                              {errors.agentRequests[index].franchisee.message}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          <input
                            {...register(`agentRequests.${index}.month_year`, {
                            ...
                            className={`w-full px-4 py-2 rounded-lg border 
                                      ${errors.agentRequests?.[index]?.month_year ? 'border-red-500' : 'border-gray-200 dark:border-gray-600'}
                                      bg-white dark:bg-gray-800 dark:text-white
                                      focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent`}
                          />
                          {errors.agentRequests?.[index]?.month_year && (
                            <p className="mt-1 text-sm text-red-500 dark:text-red-400">
                              {errors.agentRequests[index].month_year.message}
                            </p>
                          )}
                        </div>
                        ...
                        <div>
                          <input
                            {...register(`agentRequests.${index}.expected_amount`, {
                            ...
                            className={`w-full px-4 py-2 rounded-lg border 
                                      ${errors.agentRequests?.[index]?.expected_amount ? 'border-red-500' : 'border-gray-200 dark:border-gray-600'}
                                      bg-white dark:bg-gray-800 dark:text-white
                                      focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent`}
                          />
                          {errors.agentRequests?.[index]?.expected_amount && (
                            <p className="mt-1 text-sm text-red-500 dark:text-red-400">
                              {errors.agentRequests[index].expected_amount.message}
                            </p>
                          )}
                        </div>
                        ...
                        <div>
                          <input
                            {...register(`agentRequests.${index}.current_paid`)}
                            type="number"
                            step="0.01"
                  ...
                  >
                    ...
                    <span>Add Another Agent Request</span>
                  </button>
                </div>
              </motion.div>
              ...
              >
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Please provide any additional information that might help process your commission request"
                  className={`w-full px-4 py-2 rounded-lg border 
...
export default AgentCommissionRequest;
